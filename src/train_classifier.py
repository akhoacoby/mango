...
# ======================================
class MultiInputMultiOutputDataGenerator(tf.keras.utils.Sequence):
    def __init__(self, X_img, X_feat, y_dict, batch_size, datagen):
        self.X_img = np.array(X_img)
        self.X_feat = np.array(X_feat)
        self.y_dict = y_dict
        self.batch_size = batch_size
        self.datagen = datagen
        self.indexes = np.arange(len(self.X_img))
    ...
    def __getitem__(self, idx):
        ...
        batch_X_img = self.X_img[batch_indexes]
        batch_X_feat = self.X_feat[batch_indexes]

        # Apply augmentation to images
        batch_X_img_aug = np.array(
            [self.datagen.random_transform(img) for img in batch_X_img]
        )
        ...
        return [batch_X_img_aug, batch_X_feat], batch_y
...
feat_df = pd.read_csv("mango_features_new.csv")
feat_df.set_index("image", inplace=True)

# Make sure labels and features align by image name
df = df[df["image"].isin(feat_df.index)]  # Keep only valid ones
features = feat_df.loc[df["image"]].values  # in same order

# Normalize features
X_feat = np.array(features)  # shape (n_samples, n_features)
...
(
    ...
    X, X_feat, y_color, y_shape, y_quality, test_size=0.2, random_state=42
)
...
# Feature vector input branch
feat_input = Input(shape=(X_feat.shape[1],), name="feature_input")
feat_dense = Dense(64, activation="relu")(feat_input)

# Combine both
combined = Concatenate()([x, feat_dense])
combined = Dense(256, activation="relu")(combined)
combined = Dropout(0.5)(combined)


# Output heads
output_color = Dense(y_color.shape[1], activation="softmax", name="color")(combined)
output_shape = Dense(y_shape.shape[1], activation="softmax", name="shape")(combined)
output_quality = Dense(y_quality.shape[1], activation="softmax", name="quality")(
    combined
)

# Model
model = Model(
    inputs=[inputs, feat_input], outputs=[output_color, output_shape, output_quality]
)
model.compile(
...
train_generator = MultiInputMultiOutputDataGenerator(
    X_train,
    X_feat_train,
